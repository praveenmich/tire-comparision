...
export interface TireComparisonData {
  tyres: TireData[];
  serverUrl?: string;
}

// Helper to extract comparison data from any output shape
export function extractComparisonData(output: any): TireComparisonData | null {
  ...
  const serverUrl =
    output?.serverUrl ||
    data?.serverUrl ||
    output?.structuredContent?.serverUrl ||
    "http://localhost:3000";

  if (data?.tyres && data.tyres.length > 0) {
    return { tyres: data.tyres, serverUrl };
  }

  return null;
}

// Pure reusable component — no Skybridge hooks
export const TireComparisonView: React.FC<TireComparisonData> = ({
  tyres,
  ...
  useEffect(() => {
    console.log("🔍 Extracted tires for comparison:", tyres);
    if (tyres && tyres.length > 0) {
      tyres.forEach((tire: TireData, index: number) => {
      ...
      setSelectedTires(tyres.slice(0, 2));
    }
  }, [tyres, serverUrl]);
  ...
  const getProxyImageUrl = (originalUrl?: string) => {
    ...
    return `${serverUrl}/api/image-proxy?url=${encodeURIComponent(originalUrl)}`;
  };
